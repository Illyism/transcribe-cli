...
- **Quieter CLI output**: Less noise for single-chunk jobs (skip chunking chatter), silent temp cleanup, and a single saved-line with language/duration in folder mode.
...
### Added
- **Universal URL Support**: Support for transcribing any media URLs supported by `yt-dlp` (e.g. X/Twitter, and more), expanded from just YouTube.
...
### Changed
- Improved file handling in transcription process
  - Now uses file buffer and OpenAI SDK's `toFile` helper for more reliable file uploads
  - Better compatibility with different file systems and edge cases
