...
function parseSizeToBytes(input: string): number | null {
  const m = input
    .trim()
    .match(/^([\d.]+)\s*(B|bytes|KiB|kB|KB|MiB|MB|GiB|GB)?$/i);
  if (!m) return null;
...
export class DownloadProgress {
  ...
  handleLine(line: string): void {
    ...
    if (pct) {
      this.updateFromClassicPercent(
        Math.min(100, Math.max(0, parseFloat(pct[1])))
      );
    }
  ...
  finish(success: boolean): void {
    ...
    if (this.liveActive) {
      ...
      if (success) {
        console.log(
          `✅ Download complete! (${
            this.currentBytes ? formatBytes(this.currentBytes) : "done"
          })`
        );
      }
    } else if (success) {
  ...
  private renderLive(): void {
    ...
    const sizePart =
      this.currentBytes > 0 ? formatBytes(this.currentBytes) : "…";
